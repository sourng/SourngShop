...
class ccart_list extends ccart {
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fcartlist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_cart\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_cart',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fcartlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
<?php
	...
	if ($cart->CurrentAction == "" && $cart_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$cart_list->setWarningMessage(ew_DeniedMsg());
...
<?php if ($cart_list->TotalRecs > 0 || $cart->CurrentAction == "gridedit") { ?>
