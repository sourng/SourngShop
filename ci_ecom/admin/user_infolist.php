...
class cuser_info_list extends cuser_info {
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.fuser_infolist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
...
fuser_infolist.Lists["x_address2"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fuser_infolist.Lists["x_address2"].Options = <?php echo json_encode($user_info_list->address2->Options()) ?>;
...
<?php
	...
	if ($user_info->CurrentAction == "" && $user_info_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$user_info_list->setWarningMessage(ew_DeniedMsg());
...
<?php if ($user_info_list->TotalRecs > 0 || $user_info->CurrentAction == "gridedit") { ?>
...
<?php if ($user_info->address2->Visible) { // address2 ?>
	...
	<?php } else { ?>
		<th data-name="address2" class="<?php echo $user_info->address2->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $user_info->SortUrl($user_info->address2) ?>',1);"><div id="elh_user_info_address2" class="user_info_address2">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $user_info->address2->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($user_info->address2->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($user_info->address2->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
<?php
	...
	if ($user_info->CurrentAction <> "gridadd")
		$user_info_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
