...
class corders extends cTable {
	...
	function __construct() {
		...
		// user_id
		$this->user_id = new cField('orders', 'orders', 'x_user_id', 'user_id', '`user_id`', '`user_id`', 3, -1, FALSE, '`user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->user_id->Sortable = TRUE; // Allow sort
		$this->user_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['user_id'] = &$this->user_id;

		// product_id
		$this->product_id = new cField('orders', 'orders', 'x_product_id', 'product_id', '`product_id`', '`product_id`', 3, -1, FALSE, '`product_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// qty
		$this->qty = new cField('orders', 'orders', 'x_qty', 'qty', '`qty`', '`qty`', 3, -1, FALSE, '`qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->qty->Sortable = TRUE; // Allow sort
		$this->qty->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['qty'] = &$this->qty;

		// trx_id
		$this->trx_id = new cField('orders', 'orders', 'x_trx_id', 'trx_id', '`trx_id`', '`trx_id`', 200, -1, FALSE, '`trx_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->trx_id->Sortable = TRUE; // Allow sort
		$this->fields['trx_id'] = &$this->trx_id;

		// p_status
		$this->p_status = new cField('orders', 'orders', 'x_p_status', 'p_status', '`p_status`', '`p_status`', 200, -1, FALSE, '`p_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->p_status->Sortable = TRUE; // Allow sort
		$this->fields['p_status'] = &$this->p_status;
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->trx_id->setDbValue($rs->fields('trx_id'));
		$this->p_status->setDbValue($rs->fields('p_status'));
	}
	...
	function RenderEditRow() {
		...
		$this->trx_id->EditValue = $this->trx_id->CurrentValue;
		...
		$this->p_status->EditValue = $this->p_status->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->trx_id->Exportable) $Doc->ExportCaption($this->trx_id);
					if ($this->p_status->Exportable) $Doc->ExportCaption($this->p_status);
				} else {
					if ($this->order_id->Exportable) $Doc->ExportCaption($this->order_id);
					if ($this->user_id->Exportable) $Doc->ExportCaption($this->user_id);
					if ($this->product_id->Exportable) $Doc->ExportCaption($this->product_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->trx_id->Exportable) $Doc->ExportField($this->trx_id);
						if ($this->p_status->Exportable) $Doc->ExportField($this->p_status);
					} else {
						if ($this->order_id->Exportable) $Doc->ExportField($this->order_id);
						if ($this->user_id->Exportable) $Doc->ExportField($this->user_id);
						if ($this->product_id->Exportable) $Doc->ExportField($this->product_id);
