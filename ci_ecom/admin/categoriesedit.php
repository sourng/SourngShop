...
class ccategories_edit extends ccategories {
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			if (!is_null($this->cat_id->CurrentValue) && !is_null($this->brand_id->CurrentValue)) {
				while (!$this->Recordset->EOF) {
					if (strval($this->cat_id->CurrentValue) == strval($this->Recordset->fields('cat_id')) && strval($this->brand_id->CurrentValue) == strval($this->Recordset->fields('brand_id'))) {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($categories->brand_id->FldErrMsg()) ?>");
...
<?php } ?>
<?php if ($categories->brand_id->Visible) { // brand_id ?>
	<div id="r_brand_id" class="form-group">
		<label id="elh_categories_brand_id" for="x_brand_id" class="<?php echo $categories_edit->LeftColumnClass ?>"><?php echo $categories->brand_id->FldCaption() ?></label>
		<div class="<?php echo $categories_edit->RightColumnClass ?>"><div<?php echo $categories->brand_id->CellAttributes() ?>>
<span id="el_categories_brand_id">
<span<?php echo $categories->brand_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $categories->brand_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="categories" data-field="x_brand_id" name="x_brand_id" id="x_brand_id" value="<?php echo ew_HtmlEncode($categories->brand_id->CurrentValue) ?>">
<?php echo $categories->brand_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($categories->cat_title->Visible) { // cat_title ?>
	<div id="r_cat_title" class="form-group">
		<label id="elh_categories_cat_title" for="x_cat_title" class="<?php echo $categories_edit->LeftColumnClass ?>"><?php echo $categories->cat_title->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $categories_edit->RightColumnClass ?>"><div<?php echo $categories->cat_title->CellAttributes() ?>>
<span id="el_categories_cat_title">
<textarea data-table="categories" data-field="x_cat_title" name="x_cat_title" id="x_cat_title" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($categories->cat_title->getPlaceHolder()) ?>"<?php echo $categories->cat_title->EditAttributes() ?>><?php echo $categories->cat_title->EditValue ?></textarea>
</span>
<?php echo $categories->cat_title->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$categories_edit->IsModal) { ?>
