...
class ccategories extends cTable {
	...
	function __construct() {
		...
		// brand_id
		$this->brand_id = new cField('categories', 'categories', 'x_brand_id', 'brand_id', '`brand_id`', '`brand_id`', 3, -1, FALSE, '`brand_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// cat_title
		$this->cat_title = new cField('categories', 'categories', 'x_cat_title', 'cat_title', '`cat_title`', '`cat_title`', 201, -1, FALSE, '`cat_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->cat_title->Sortable = TRUE; // Allow sort
		$this->fields['cat_title'] = &$this->cat_title;
	}
	...
	function SqlKeyFilter() {
		return "`cat_id` = @cat_id@ AND `brand_id` = @brand_id@";
	}
	...
	function GetRecordKeys() {
		...
		if (is_array($arKeys)) {
			foreach ($arKeys as $key) {
				...
				if (!is_numeric($key[0])) // cat_id
					continue;
				if (!is_numeric($key[1])) // brand_id
					continue;
				$ar[] = $key;
			}
		}
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->cat_id->CurrentValue = $key[0];
	...
	function LoadListRowValues(&$rs) {
		...
		$this->cat_title->setDbValue($rs->fields('cat_title'));
	}
	...
	function RenderEditRow() {
		...
		$this->cat_title->EditValue = $this->cat_title->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->cat_title->Exportable) $Doc->ExportCaption($this->cat_title);
				} else {
					if ($this->cat_id->Exportable) $Doc->ExportCaption($this->cat_id);
					if ($this->brand_id->Exportable) $Doc->ExportCaption($this->brand_id);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->cat_title->Exportable) $Doc->ExportField($this->cat_title);
					} else {
						if ($this->cat_id->Exportable) $Doc->ExportField($this->cat_id);
						if ($this->brand_id->Exportable) $Doc->ExportField($this->brand_id);
					}
